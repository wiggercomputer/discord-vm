...
def main():
    parser = argparse.ArgumentParser(description='Send a voice message to a Discord channel.')
    parser.add_argument('--token', type=str, required=True, help='Your Discord token.')
    parser.add_argument('--channel', type=int, required=True, help='The ID of the Discord channel.')
    parser.add_argument('--file', type=str, required=True, help='The path to the voice message file.')
    args = parser.parse_args()
    
    voicemessage = VoiceMessageSender(args.token, args.channel)
    voicemessage.SendVoiceMessage(args.file)
    ...
    def __GetUrl(self, file_path):
        ...
        with open(file_path, 'rb') as file:
            ...
            if 'attachments' in response_data:
                ...
                raise Exception(f"Attachment URL returned {response_data}")
